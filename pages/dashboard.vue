  ...
  <div class="min-h-screen bg-white dark:bg-slate-950 transition-colors duration-500">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="mb-10 animate-fade-in">
        ...
        <div
          class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-400 text-xs font-bold uppercase tracking-wider">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
          </svg>
          Recordatorio: No es asesoría legal. Revisa los resultados con un profesional.
        </div>
      </div>
      ...
      <div class="grid md:grid-cols-3 gap-6 mb-8">
        <div
          class="bg-white dark:bg-slate-900 rounded-3xl shadow-soft p-6 border border-slate-100 dark:border-slate-800 transition-all hover:shadow-premium group">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider text-xs">Total Análisis
            </h3>
            <div
              class="w-12 h-12 bg-secondary/10 rounded-2xl flex items-center justify-center text-secondary group-hover:scale-110 transition-transform">
        ...
        <div
          class="bg-white dark:bg-slate-900 rounded-3xl shadow-soft p-6 border border-slate-100 dark:border-slate-800 transition-all hover:shadow-premium group">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider text-xs">Créditos
              Disponibles</h3>
            <div
              class="w-12 h-12 bg-secondary/10 rounded-2xl flex items-center justify-center text-secondary group-hover:scale-110 transition-transform">
              <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div
          class="bg-white dark:bg-slate-900 rounded-3xl shadow-soft p-6 border border-slate-100 dark:border-slate-800 transition-all hover:shadow-premium group">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider text-xs">Último Análisis
            </h3>
            <div
              class="w-12 h-12 bg-risk-medium/10 rounded-2xl flex items-center justify-center text-risk-medium group-hover:scale-110 transition-transform">
              <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd"
      ...
      <div
        class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-premium p-8 mb-12 border border-slate-100 dark:border-slate-800 relative overflow-hidden">
        ...
        <div v-if="selectedFile" class="space-y-8 animate-slide-up">
          ...
          <div class="flex flex-col sm:flex-row gap-4">
            <input v-model="contractName" type="text" placeholder="Nombre del contrato (ej: Contrato de Arrendamiento)"
        ...
        <div v-if="(userProfile?.credits || 0) < 1 && !selectedFile"
          class="mt-4 p-4 bg-risk-medium/10 border border-risk-medium rounded-lg">
      ...
      <!-- Recent Analyses -->
      <div
        class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-premium p-8 border border-slate-100 dark:border-slate-800">
        ...
        <div v-else-if="analyses.length === 0"
          class="text-center py-20 bg-slate-50 dark:bg-slate-950/50 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800">
          <svg class="w-16 h-16 text-slate-300 dark:text-slate-700 mx-auto mb-4" fill="none" stroke="currentColor"
            viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div v-else class="space-y-4">
          <NuxtLink v-for="analysis in analyses" :key="analysis.id" :to="`/analyze/${analysis.id}`"
            ...
            <div class="flex items-start justify-between">
              <div class="flex-1">
                <h3
                  class="text-lg font-bold text-slate-900 dark:text-white mb-2 group-hover/item:text-secondary transition-colors">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <template v-else-if="analysis.status === 'processing'">
                    <span
                      class="px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-secondary/10 text-secondary flex items-center gap-2">
                  ...
                  <template v-else-if="analysis.status === 'failed'">
                    <span
                      class="px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-risk-high/10 text-risk-high">
                      Error en análisis
                    </span>
                  </template>
                  <template v-else>
                    <span
                      class="px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-slate-200 dark:bg-slate-800 text-slate-500">
                      En cola...
                    </span>
                  </template>
                </div>
              ...
              <div
                class="w-10 h-10 rounded-xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex items-center justify-center text-slate-400 group-hover/item:text-secondary group-hover/item:border-secondary/30 transition-all">
