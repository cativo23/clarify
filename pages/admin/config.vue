  ...
  try {
    ...
    if (data && data.models) {
      config.value = {
        ...
        tiers: {
          basic: {
            model: data.models.basic || "",
            credits: 1,
            tokenLimits: data.tokenLimits?.basic || {
              input: 8000,
              output: 2500,
            },
          },
          premium: {
            model: data.models.premium || "",
            credits: 3,
            tokenLimits: data.tokenLimits?.premium || {
              input: 35000,
              output: 10000,
            },
          },
  ...
  <div class="w-full animate-slide-up">
    ...
    <div class="mb-12">
      ...
      <p class="text-slate-500 dark:text-slate-400 font-medium">
        Gestiona parámetros globales de la IA y límites.
      </p>
    </div>
    ...
    <div v-if="loading" class="py-20 text-center">
      ...
      <p
        class="mt-6 text-slate-400 font-bold uppercase tracking-widest text-xs"
      >
    ...
    <div v-else class="space-y-8 animate-slide-up">
      ...
      <!-- Prompt Version -->
      <section
        class="bg-white dark:bg-slate-900 p-8 rounded-[2rem] shadow-premium border border-slate-100 dark:border-slate-800"
      >
        <div class="flex items-start justify-between mb-6">
          <div>
            <h2 class="text-xl font-black mb-1">Versión de Prompts</h2>
            <p class="text-slate-500 text-sm font-medium">
              Controla qué versión de instrucciones utiliza la IA.
            </p>
          </div>
          <span
            class="px-3 py-1 bg-secondary/10 text-secondary text-[10px] font-black uppercase rounded-lg"
            >Critical</span
          >
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-4">
          <label
            :class="[
              'cursor-pointer lg:col-span-1 p-4 rounded-2xl border-2 transition-all flex items-center gap-4',
          ...
          >
            ...
            <div>
              ...
              <span class="text-xs text-slate-500"
                >Antigua estructura. No recomendado.</span
              >
            </div>
          </label>
          ...
          >
            ...
            <div>
              ...
              <span class="text-xs text-slate-500"
                >Formato JSON estricto y depuración.</span
              >
            </div>
          </label>
        </div>
      ...
      <div class="grid lg:grid-cols-2 gap-8">
          ...
          <div class="space-y-6">
            ...
            >
              ...
              <div
                ...
                @click="showDropdownBasic = !showDropdownBasic"
              >
                <span
                  :class="
                    config.tiers?.basic?.model
                ...
                <svg
                  class="w-4 h-4 text-slate-400 group-hover:text-secondary transition-transform"
                  :class="{ 'rotate-180': showDropdownBasic }"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <div
                v-if="showDropdownBasic"
                class="absolute z-20 mt-2 w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl overflow-hidden p-2 animate-slide-up"
              >
                <div class="mb-2 p-2 relative">
                  ...
                  />
                  <svg
                    class="w-3.5 h-3.5 absolute left-5 top-1/2 -translate-y-1/2 text-slate-400"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                >
                  ...
                  >
                    <span>{{ m.name }}</span>
                    <span
                      class="text-[8px] px-1.5 py-0.5 rounded uppercase"
                      :class="
                        m.type === 'reasoning'
            ...
            <div
              v-click-outside="() => (showDropdownPremium = false)"
              class="relative"
            >
              <label
                class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2"
                >Premium Plan Model</label
              >
              <div
                class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 font-mono text-sm cursor-pointer flex justify-between items-center group hover:border-secondary transition-all"
                @click="showDropdownPremium = !showDropdownPremium"
              >
                <span
                  :class="
                    config.tiers?.premium?.model
                      ? 'text-slate-900 dark:text-white'
                      : 'text-slate-400'
                  "
                >
                ...
                <svg
                  class="w-4 h-4 text-slate-400 group-hover:text-secondary transition-transform"
                  :class="{ 'rotate-180': showDropdownPremium }"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              >
                ...
                >
                  <button
                    v-for="m in filteredModelsPremium"
                    :key="m.id"
                    class="w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-between group"
                    :class="
                      config.tiers?.premium?.model === m.id
                        ? 'bg-secondary text-white'
                        : 'hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400'
                    "
            ...
            <div
              v-click-outside="() => (showDropdownForensic = false)"
              class="relative"
            >
              <label
                class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2"
                >Forensic Plan Model</label
              >
              <div
                class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 font-mono text-sm cursor-pointer flex justify-between items-center group hover:border-secondary transition-all"
                @click="showDropdownForensic = !showDropdownForensic"
              >
                <span
                  :class="
                    config.tiers?.forensic?.model
                      ? 'text-slate-900 dark:text-white'
                      : 'text-slate-400'
                  "
                >
                ...
                <svg
                  class="w-4 h-4 text-slate-400 group-hover:text-secondary transition-transform"
                  :class="{ 'rotate-180': showDropdownForensic }"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <div
                v-if="showDropdownForensic"
                class="absolute z-20 mt-2 w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl overflow-hidden p-2 animate-slide-up"
              >
                <div class="mb-2 p-2 relative">
                  <input
                    v-model="searchForensic"
                    type="text"
                    placeholder="Buscar modelo..."
                    class="w-full bg-slate-100 dark:bg-slate-800 border-none rounded-lg px-8 py-2 text-xs outline-none"
                  />
                ...
                >
                  <button
                    v-for="m in filteredModelsForensic"
                    :key="m.id"
                    class="w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-between group"
                    :class="
                      config.tiers?.forensic?.model === m.id
                        ? 'bg-secondary text-white'
                        : 'hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400'
                    "
        ...
        >
          ...
          <div class="space-y-4">
            <label
              class="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 cursor-pointer hover:border-secondary/50 transition-all"
            >
              <div>
                <span class="block font-bold text-sm">Preprocessing Smart</span>
                <span class="text-xs text-slate-500"
                  >Análisis inteligente de secciones</span
                >
              </div>
              <div
              ...
              >
                <input
                  v-model="config.features.preprocessing"
                  type="checkbox"
                  class="toggle-checkbox absolute block w-6 h-6 rounded-full bg-white border-4 appearance-none cursor-pointer text-secondary checked:right-0 right-6 checked:bg-secondary checked:border-secondary"
                />
                <label
                  class="toggle-label block overflow-hidden h-6 rounded-full bg-slate-300 dark:bg-slate-800 cursor-pointer"
                ></label>
              </div>
            </label>
            ...
            >
              <div>
                ...
                <span class="text-xs text-slate-500"
                  >Logs detallados de tokens y errores</span
                >
              </div>
              <div
                class="relative inline-block w-12 mr-2 align-middle select-none transition duration-200 ease-in"
              >
                <input
                  v-model="config.features.tokenDebug"
                  type="checkbox"
                  class="toggle-checkbox absolute block w-6 h-6 rounded-full bg-white border-4 appearance-none cursor-pointer text-secondary checked:right-0 right-6 checked:bg-secondary checked:border-secondary"
                />
                <label
      ...
      >
        ...
        <div class="grid md:grid-cols-3 gap-8">
          <div
            class="p-6 bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800"
          >
            <h3
            ...
            <div class="space-y-4">
              <div>
                ...
                >
                <input
                  v-model="config.tiers.basic.credits"
                  type="number"
                  class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 text-sm font-bold"
                />
              </div>
              <div>
                <label
                ...
                >
                <input
                  v-model="config.tiers.basic.tokenLimits.input"
                  type="number"
                  class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 text-sm font-bold"
                />
              </div>
              <div>
                ...
                >
                <input
                  v-model="config.tiers.basic.tokenLimits.output"
                  type="number"
                  class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 text-sm font-bold"
                />
              </div>
          ...
          >
            ...
            <div class="space-y-4">
              <div>
                ...
                >
                <input
                  v-model="config.tiers.forensic.credits"
                  type="number"
                  class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 text-sm font-bold"
                />
              </div>
              <div>
                ...
                >
                <input
                  v-model="config.tiers.forensic.tokenLimits.input"
                  type="number"
                  class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 text-sm font-bold"
                />
              </div>
              <div>
                ...
                >
                <input
                  v-model="config.tiers.forensic.tokenLimits.output"
                  type="number"
                  class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 text-sm font-bold"
                />
              </div>
          ...
          >
            ...
            <div class="space-y-4">
              <div>
                ...
                >
                <input
                  v-model="config.tiers.premium.credits"
                  type="number"
                  class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 text-sm font-bold"
                />
              </div>
              <div>
                ...
                >
                <input
                  v-model="config.tiers.premium.tokenLimits.input"
                  type="number"
                  class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 text-sm font-bold"
                />
              </div>
              <div>
                ...
                >
                <input
                  v-model="config.tiers.premium.tokenLimits.output"
                  type="number"
                  class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 text-sm font-bold"
                />
              </div>
