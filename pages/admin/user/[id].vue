...
const highRiskCount = computed(
  () => analyses.value.filter((a) => a.risk_level === "high").length,
);
const mediumRiskCount = computed(
  () => analyses.value.filter((a) => a.risk_level === "medium").length,
);
      ...
      <div v-else-if="profile" class="space-y-8">
        ...
        >
          <div class="flex items-center gap-6">
            <div
              class="w-20 h-20 bg-gradient-to-br from-secondary to-accent-indigo rounded-3xl flex items-center justify-center text-white text-3xl font-black shadow-lg"
            >
            ...
            <div class="flex-1">
              ...
              <div class="flex items-center gap-4">
                ...
                >
                <span
                  class="w-1 h-1 bg-slate-300 dark:bg-slate-700 rounded-full"
                ></span>
                <span
                  class="text-[10px] font-bold text-slate-400 uppercase tracking-widest"
                  >Joined {{ formatDate(profile.created_at) }}</span
                >
              </div>
            </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          >
            <div class="flex items-center gap-3 mb-3">
              <div
                class="w-10 h-10 bg-risk-high/10 rounded-xl flex items-center justify-center text-risk-high"
              >
                <svg
                  class="w-5 h-5"
                  fill="none"
            ...
            <p class="text-3xl font-black text-risk-high">
              {{ highRiskCount }}
            </p>
          </div>
        ...
        >
          <div class="flex items-center justify-between mb-6">
            <div>
              ...
              <p class="text-sm font-bold text-slate-500 dark:text-slate-400">
                All contract analyses for this user
              </p>
            </div>
            <div class="flex items-center gap-2">
              <span
                class="text-[10px] font-black text-slate-400 uppercase tracking-widest"
                >Low: {{ lowRiskCount }}</span
              >
              <span
                class="w-1 h-1 bg-slate-300 dark:bg-slate-700 rounded-full"
              ></span>
          ...
          <div
            ...
            class="text-center py-12 bg-slate-50 dark:bg-slate-800/50 rounded-2xl"
          >
            <svg
              class="w-16 h-16 text-slate-200 dark:text-slate-800 mx-auto mb-4"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <p class="text-slate-900 dark:text-white font-black mb-1">
              No analyses yet
            </p>
            <p class="text-slate-500 dark:text-slate-500 text-sm font-medium">
              This user hasn't analyzed any contracts.
            </p>
          </div>
          ...
          <div v-else class="space-y-3">
            ...
            >
              <div class="flex items-start justify-between gap-4 mb-4">
                <div class="flex items-center gap-3">
                  <NuxtLink
                    :to="`/analyze/${a.id}`"
                    :class="[
                      'w-12 h-12 rounded-2xl flex items-center justify-center transition-all hover:scale-105',
                  ...
                  >
                    ...
                    <span v-else class="text-xs font-black uppercase">{{
                      a.status
                    }}</span>
                  </NuxtLink>
                  <div>
              ...
              <details class="group">
                <summary
                  class="flex items-center gap-2 cursor-pointer text-xs font-bold text-slate-400 hover:text-secondary transition-colors"
                >
                  <svg
                ...
                <pre
                  class="mt-3 p-4 bg-slate-900 dark:bg-slate-950 rounded-xl overflow-auto text-xs font-mono text-slate-300 max-h-96"
                  >{{ JSON.stringify(a.summary_json, null, 2) }}</pre
                >
              </details>
            </div>
          </div>
