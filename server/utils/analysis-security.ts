 ...
 */
export function sanitizeAnalysisSummary(
  summary: any,
  isAdmin: boolean,
  tokenDebugEnabled: boolean = false,
): any {
 ...
 */
export function sanitizeAnalysesList(
  analyses: Analysis[],
  isAdmin: boolean,
  tokenDebugEnabled: boolean = false,
): Analysis[] {
  if (!isAdmin && !tokenDebugEnabled) {
    return analyses.map((analysis) => ({
      ...analysis,
      summary_json: sanitizeAnalysisSummary(
...
export async function isTokenDebugEnabled(): Promise<boolean> {
  try {
    ...
    console.error(
      "[Analysis Security] Error checking tokenDebug config:",
      error,
    );
    return false;
  }
