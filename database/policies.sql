...
-- Permitir que la aplicación (Service Role) gestione todo (esto es implícito, pero buena práctica tenerlo claro)
-- Service Role siempre tiene bypass RLS, así que no se necesita policy específica para él.
...
-- Insertar transacciones (usualmente solo el backend, pero por si acaso)
-- El backend usa service role, así que no necesita policy.

-- POLÍTICAS PARA STORAGE (para el bucket 'contracts')
-- Necesitas ejecutar esto si aún no tienes políticas de storage
-- Asegúrate de que el bucket 'contracts' exista primero.
