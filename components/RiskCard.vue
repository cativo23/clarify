  ...
  <div
    ...
    <div class="flex items-center gap-6 mb-6">
      <div :class="[
        ...
        <svg v-if="risk === 'high'" class="w-7 h-7 text-white" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd"
            d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
            clip-rule="evenodd" />
        </svg>
        <svg v-else-if="risk === 'medium'" class="w-7 h-7 text-white" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd"
    ...
    <div v-if="props.clausula || props.citaTextual || props.riesgoReal || props.mitigacion || props.details">
      <button @click="isExpanded = !isExpanded"
        ...
        <span class="border-b-2 border-secondary/30 group-hover/btn:border-secondary transition-all">{{ isExpanded ?
          'Ver menos' : 'Ver más detalles' }}</span>
        <svg :class="['w-4 h-4 transition-transform duration-500', isExpanded ? 'rotate-180' : '']" fill="none"
      ...
      <div v-if="isExpanded"
        ...
        <div v-if="props.clausula">
          <span
            class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest block mb-2">Ubicación</span>
        ...
        <div v-if="props.mitigacion">
          <span class="text-[10px] font-black text-secondary uppercase tracking-widest block mb-2">Sugerencia de
            Mitigación</span>
        ...
        <div v-if="props.citaTextual">
          <span
            class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest block mb-2">Referencia
            del Contrato</span>
          <div
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 italic font-medium leading-relaxed">{{
              props.citaTextual }}</p>
          </div>
        </div>
        ...
        <div v-if="props.details">
          <span
            class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest block mb-2">Notas
            Adicionales</span>
