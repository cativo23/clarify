<template>
  <div class="bg-white dark:bg-slate-900 rounded-[2rem] shadow-soft hover:shadow-premium transition-all duration-300 p-8 border border-slate-100 dark:border-slate-800 group">
    ...
    <div class="flex items-center gap-6 mb-6">
      ...
      <div class="flex-1">
        <h3 class="text-xl font-black text-slate-900 dark:text-white mb-1">{{ category }}</h3>
    ...
    <div v-if="props.clausula || props.citaTextual || props.riesgoReal || props.mitigacion || props.details">
      ...
      >
        <span class="border-b-2 border-secondary/30 group-hover/btn:border-secondary transition-all">{{ isExpanded ? 'Ver menos' : 'Ver más detalles' }}</span>
        <svg 
          :class="['w-4 h-4 transition-transform duration-500', isExpanded ? 'rotate-180' : '']"
          fill="none" 
          stroke="currentColor" 
          viewBox="0 0 24 24"
        >
      ...
      >
        <div v-if="props.clausula">
          <span class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest block mb-2">Ubicación</span>
          <p class="text-sm text-slate-900 dark:text-slate-200 font-bold">{{ props.clausula }}</p>
        </div>

        <div v-if="props.riesgoReal">
          <span class="text-[10px] font-black text-risk-high uppercase tracking-widest block mb-2">Impacto Real</span>
          <p class="text-sm text-slate-700 dark:text-slate-300">{{ props.riesgoReal }}</p>
        </div>

        <div v-if="props.mitigacion">
          <span class="text-[10px] font-black text-secondary uppercase tracking-widest block mb-2">Sugerencia de Mitigación</span>
          <p class="text-sm text-slate-700 dark:text-slate-300">{{ props.mitigacion }}</p>
        </div>
        
        <div v-if="props.citaTextual">
          <span class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest block mb-2">Referencia del Contrato</span>
          <div class="relative mt-2 p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800">
            <svg class="absolute -left-2 -top-2 w-8 h-8 text-secondary/10" fill="currentColor" viewBox="0 0 24 24">
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 italic font-medium leading-relaxed">{{ props.citaTextual }}</p>
          </div>
        </div>

        <div v-if="props.details">
          <span class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest block mb-2">Notas Adicionales</span>
          <p class="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">{{ props.details }}</p>
        </div>
      </div>
    </div>
  </div>
