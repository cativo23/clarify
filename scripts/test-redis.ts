...
async function testRedisConnection() {
  ...
  console.log(
    `   TLS: ${isUpstash ? "✅ Enabled (rediss://)" : "❌ Disabled (local)"}`,
  );
  console.log(
    `   Auth: ${token ? "✅ Token-based (Upstash)" : "❌ None (local)"}\n`,
  );
  ...
  try {
    ...
    console.log(
      `   Mode: ${info.includes("role:master") ? "Master" : "Replica"}`,
    );
    ...
    console.log(
      "4. For Upstash: redis-cli -h <host> -p 6379 -a <token> --tls\n",
    );
    process.exit(1);
  } finally {
    await redis.quit();
