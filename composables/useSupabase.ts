...
export const useUserState = () =>
  useState<User | null>("user-profile", () => null);
// Track last fetch time for cache invalidation
export const useUserStateLastFetch = () =>
  useState<number>("user-profile-last-fetch", () => 0);
...
export const uploadContractFile = async (file: File) => {
  ...
  const {
    data: { publicUrl },
  } = client.storage.from("contracts").getPublicUrl(fileName);
